...
export class ComputerUseOpenAi {

    static queryLlm(payload, chatStatus): LlmResponse {
        ...
        const response: LlmResponse = AiServerClient.QueryRaw(payload, { defaultModelApiType: "openai" });
        chatStatus.prompt_queries += 1;
        chatStatus.input_tokens += response.usage?.input_tokens || 0;
        chatStatus.output_tokens += response.usage?.output_tokens || 0;
        return response;
    }
        ...
        while (true) {
            ...
            response.output.forEach((item) => {
                if (item.type === "reasoning"&& item.summary && item.summary.length) {
                    if (action_reasoning) action_reasoning += "\n";
            ...
            if (computerCall.pending_safety_checks && computerCall.pending_safety_checks.length > 0) {
                ...
                computerCall.pending_safety_checks.forEach((check: any) => {
                ...
                acknowledged_safety_checks = computerCall.pending_safety_checks.map((check: any) => ({
                    id: check.id,
                    code: check.code,
        ...
        try {
            switch (actionType) {
                case "click": {
                    ...
                    const [clickX, clickY] = ComputerUseUtils.applyScaling([x, y], this.imgMeta.scale_factor);
                    window.DoMouseMove(clickX, clickY);
                    let clickType: "L" | "R" | "M" = "L";
                    switch (button) {
                        case "right": clickType = "R"; break;
                        case "middle": clickType = "M"; break;
                        default: clickType = "L"; break;
                    }
                ...
                case "drag": {
                    ...
                    const [startX, startY] = ComputerUseUtils.applyScaling([start.x, start.y], this.imgMeta.scale_factor);
                    window.DoMouseMove(startX, startY);
                    window.DoMousePress("L");
                    for (const point of rest) {
                        const [dragX, dragY] = ComputerUseUtils.applyScaling([point.x, point.y], this.imgMeta.scale_factor);
                ...
                case "keypress": {
                    ...
                    const combinedKeyMap = new Map<string, string>([
                        // --- Special Keys (Common to Keys & Codes where names overlap) ---
                        [Keys.Enter, "{RETURN}"],        // Handles "Enter"
                        [Keys.Tab, "{TAB}"],           // Handles "Tab"
                        [Keys.Escape, "{ESCAPE}"],       // Handles "Escape"
                        [Keys.Backspace, "{BACKSPACE}"],   // Handles "Backspace"
                        [Keys.Delete, "{DELETE}"],       // Handles "Delete"
                        [Keys.Insert, "{INSERT}"],       // Handles "Insert"
                        [Keys.Pause, "{PAUSE}"],         // Handles "Pause"
                        [Keys.CapsLock, "{CAPS_LOCK}"],   // Handles "CapsLock"
                        [Keys.ArrowUp, "{ARROW_UP}"],    // Handles "ArrowUp"
                        [Keys.ArrowDown, "{ARROW_DOWN}"],  // Handles "ArrowDown"
                        [Keys.ArrowLeft, "{ARROW_LEFT}"],  // Handles "ArrowLeft"
                        [Keys.ArrowRight, "{ARROW_RIGHT}"], // Handles "ArrowRight"
                        [Keys.PageUp, "{PAGE_UP}"],      // Handles "PageUp"
                        [Keys.PageDown, "{PAGE_DOWN}"],   // Handles "PageDown"
                        [Keys.Home, "{HOME}"],           // Handles "Home"
                        [Keys.End, "{END}"],             // Handles "End"
                        [Keys.PrintScreen, "{PRINT_SCREEN}"], // Handles "PrintScreen"
                        [Keys.ScrollLock, "{SCROLL_LOCK}"], // Handles "ScrollLock"
                        [Keys.NumLock, "{NUM_LOCK}"],      // Handles "NumLock"
                        ...
                        [Keys.Space, " "],               // Handles " " (from Keys.Space)
                        [Codes.Space, " "],              // Handles "Space" (from Codes.Space) -> send literal space

                        // Modifier Keys (Map name to sendKeys code - chord handling is separate)
                        [Keys.Shift, "{SHIFT}"],         // Handles "Shift"
                        [Codes.ShiftLeft, "{SHIFT}"],    // Handle "ShiftLeft"
                        [Codes.ShiftRight, "{SHIFT}"],   // Handle "ShiftRight"
                        [Keys.Control, "{CONTROL}"],     // Handles "Control"
                        [Codes.ControlLeft, "{CONTROL}"], // Handle "ControlLeft"
                        [Codes.ControlRight, "{CONTROL}"], // Handle "ControlRight"
                        [Keys.Alt, "{ALT}"],             // Handles "Alt"
                        [Codes.AltLeft, "{ALT}"],        // Handle "AltLeft"
                        [Codes.AltRight, "{ALT}"],       // Handle "AltRight"
                        [Keys.Meta, "{META}"],           // Handles "Meta" (Windows/Command key)
                        [Codes.MetaLeft, "{META}"],      // Handle "MetaLeft"
                        [Codes.MetaRight, "{META}"],     // Handle "MetaRight"

                        // Media Keys (Names common in Keys & Codes)
                        [Keys.AudioVolumeMute, "{VOLUME_MUTE}"], // Handles "AudioVolumeMute"
                        [Keys.AudioVolumeDown, "{VOLUME_DOWN}"], // Handles "AudioVolumeDown"
                        [Keys.AudioVolumeUp, "{VOLUME_UP}"],   // Handles "AudioVolumeUp"



                        // Numpad Digits (Codes.Numpad5 -> "{NUMPAD5}" or "5") - Choose based on sendKeys
                        [Codes.Numpad0, "{NUMPAD0}"], // Or "0"
                        [Codes.Numpad1, "{NUMPAD1}"], // Or "1"
                        [Codes.Numpad2, "{NUMPAD2}"], // Or "2"
                        [Codes.Numpad3, "{NUMPAD3}"], // Or "3"
                        [Codes.Numpad4, "{NUMPAD4}"], // Or "4"
                        [Codes.Numpad5, "{NUMPAD5}"], // Or "5"
                        [Codes.Numpad6, "{NUMPAD6}"], // Or "6"
                        [Codes.Numpad7, "{NUMPAD7}"], // Or "7"
                        [Codes.Numpad8, "{NUMPAD8}"], // Or "8"
                        [Codes.Numpad9, "{NUMPAD9}"], // Or "9"

                        // Numpad Operators (Codes.NumpadAdd -> "{NUMPAD_ADD}" or "+")
                        [Codes.NumpadAdd, "{NUMPAD_ADD}"],        // Or "+"
                        [Codes.NumpadSubtract, "{NUMPAD_SUBTRACT}"], // Or "-"
                        [Codes.NumpadMultiply, "{NUMPAD_MULTIPLY}"], // Or "*"
                        [Codes.NumpadDivide, "{NUMPAD_DIVIDE}"],   // Or "/"
                        [Codes.NumpadDecimal, "{NUMPAD_DECIMAL}"], // Or "."
                        ...
                        // Context Menu Key
                        [Codes.ContextMenu, "{CONTEXT_MENU}"], // Check sendKeys name for this
                    ]);
                    ...
                    for (const k of keys) { // k is a string here
                        ...
                        const caseInsensitiveKeyMap = new Map<string, string>();
                ...
                case "type": {
                    ...
                    window.DoSendKeys(ComputerUseUtils.escapeSendKeysSpecialChars(text), 10);
                    break;
                }
